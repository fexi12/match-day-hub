...
function getPostLoginDestination() {
  const raw = window.localStorage.getItem(AUTH_RETURN_TO_KEY);
...
function LoginPage() {
  ...
  useEffect(() => {
    if (!loading && user) navigate({ to: getPostLoginDestination() });
      ...
      <div className="w-full max-w-md border-2 border-primary rounded-xl p-8 bg-card">
        ...
        >
          <svg className="h-4 w-4 mr-2" viewBox="0 0 48 48" aria-hidden>
            ...
            <path
              fill="#FF3D00"
              d="M6.3 14.7l6.6 4.8C14.6 16 18.9 13.5 24 13.5c2.9 0 5.6 1.1 7.7 2.9l5.7-5.7C33.6 7.3 29 5.5 24 5.5c-7.4 0-13.8 4.2-17.7 9.2z"
            />
            <path
              fill="#4CAF50"
              d="M24 43.5c5 0 9.5-1.7 12.9-4.5l-6-5c-1.9 1.3-4.3 2-6.9 2-5.2 0-9.6-3.1-11.2-7.4l-6.5 5C9.7 39.6 16.3 43.5 24 43.5z"
            />
            <path
              fill="#1976D2"
              d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.2 4.1-4 5.5l6 5c-.4.4 6.7-4.9 6.7-14.5 0-1.2-.1-2.3-.4-3.5z"
            />
          </svg>

...
BEGIN
  ...
  _slot_email := lower(coalesce(_slot ->> 'email', ''));

  -- A normal authenticated player can replace an empty/manual name-only slot,
  -- but cannot steal a slot already claimed by another Google account.
  IF _slot_email <> '' AND _slot_email <> _email THEN
    RAISE EXCEPTION 'Slot already claimed by another account';
  END IF;
